...
--4) Obtener un listado de los empleados y los servicios a los que estan asignados, exclusivamente para aquellos que tengan algun servicio asignado.
/*select	empleado.* 
from	public.empleado
join 	servicio on servicio.cods = empleado.cods*/

--5) Obtener el numero de habitacion, tipo y precio de las habitaciones que esten ocupadas en la actualidad (no tienen fecha de salida).
...
--6) Obtener el nombre y apellidos del cliente o clientes que mas veces hayan estado en el hotel (no numero total de noches, sino estancias diferentes: una persona que ha estado tres veces diferentes de una noche en el hotel habra "estado" mas veces que otra persona que ha estado una vez durante tres noches)
/*with ms as(
...
--9) Obtener un listado de los proveedores cuyas facturas hayan sido aprobadas unicamente por el encargado y no por el responsable de un servicio.
/*select	factura_prov.*
from	proveedor
...
--10) Igualar el sueldo del empleado que menos cobra dentro del servicio de "restaurante" con el del empleado que mas cobra del mismo servicio.
/*with mmm as(
...
--11) Incrementar en un 10% el sueldo del empleado de "limpieza" que maÌs habitaciones haya limpiado.
/*
with cleaner as (
